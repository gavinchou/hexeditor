...
void saveSettings(void)
{
	...
	//new added 
	::WritePrivateProfileString(dlgEditor, rgbLeadIn, _itot(prop.colorProp.rgbLeadIn, temp, 10), iniFilePath);
	::WritePrivateProfileString(dlgEditor, rgbMetaData, _itot(prop.colorProp.rgbMetaData, temp, 10), iniFilePath);
	::WritePrivateProfileString(dlgEditor, rgbRawData, _itot(prop.colorProp.rgbRawData, temp, 10), iniFilePath);
...
COLORREF getColor(eColorType type)
{
	switch (type)
	{
		...
		//new added
		case HEX_COLOR_LEADIN:
			return prop.colorProp.rgbLeadIn;
		case HEX_COLOR_METADATA:
			return prop.colorProp.rgbMetaData;
		case HEX_COLOR_RAWDATA:
			return prop.colorProp.rgbRawData;
	}
	return 0;
}
